		 ...
		 $.ajax({
		    	...
		    	,success:function(data){
		    		...
		    		if(data.customerDTO){ //DB에서 넘어온 데이터가 입력폼에 입력받은 데이터와 같다면 
		    			//alert("이미 사용중인 아이디입니다. 다른아이디를 입력해 주세요");
		    	...
		    	error:function(data){
		    		if(data.customerInfoDto != userId){
		    			//alert("사용가능한 아이디 입니다!(^*^)☆")
	...
	$("#btnEmailCheck").click(function(){
		...
		console.log(userEmail);
		 ...
		 function eventSuccess(data){
			 alert("데이터보내기 성공!");
			 window.open(target,'emailCheck',settings);
		 }
...
<body>
	<!-- 내가만든 모달 회원가입 폼 -->
			...
			<!-- 회원가입 입력 폼 -->
			<form:form name="joinForm" commandName="joinForm" method="post">
				...
				<div class="form-group">
						...
						<input type="button" class="btn btn-lg btn-primary btn-block-jointext" value="이메일인증"  name="email" id="btnEmailCheck">
				</div>
				<div class="form-group">
						<label for="code">☆메일인증여부☆</label><br>
						<input type="text" name="mailCheck" id="mailCheck" placeholder="메일인증여부확인란입니다~" class="form-joinButton" required />
				</div>
